  ...
  return (
    <section id="skills" className="pt-section-gap">
      <div className="text-center mb-stack-lg">
        ...
        <h2 className="font-h1 text-xl text-on-surface">Tech Stack</h2>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-8 gap-4 max-w-5xl mx-auto">
        {skills?.map((skill) => (
          <div
            key={skill.name}
            className="group relative flex flex-col items-center gap-3 p-4 rounded-2xl glass-panel hover:bg-primary/5 hover:-translate-y-1 transition-all duration-300 cursor-default"
          >
            {/* Tooltip Info Box */}
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-52 p-4 rounded-xl glass-panel-high opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-[5000] pointer-events-none translate-y-2 group-hover:translate-y-0">
              <div className="flex flex-col gap-2 text-[11px] text-on-surface">
              ...
              {/* Tooltip Arrow */}
              <div className="absolute top-[calc(100%-8px)] left-1/2 -translate-x-1/2 w-4 h-4 bg-surface-container border-r border-b border-outline-variant/30 rotate-45 backdrop-blur-xl -z-10"></div>
            </div>
            ...
            <span className="text-[11px] text-center text-on-surface-variant group-hover:text-on-surface transition-colors duration-300 architectural-tracking font-medium leading-tight mt-1 relative z-10">
              {skill.name}
            </span>
          </div>
        ))}
